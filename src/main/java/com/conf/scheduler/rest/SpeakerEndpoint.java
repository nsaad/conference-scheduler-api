...
import com.conf.scheduler.model.Speaker;
...
public class SpeakerEndpoint
{
   ...
   {
      TypedQuery<Speaker> findByIdQuery = em.createQuery("SELECT DISTINCT s FROM Speaker s LEFT JOIN FETCH s.presentations WHERE s.id = :entityId ORDER BY s.id", Speaker.class);
   ...
   {
      TypedQuery<Speaker> findAllQuery = em.createQuery("SELECT DISTINCT s FROM Speaker s LEFT JOIN FETCH s.presentations ORDER BY s.id", Speaker.class);
